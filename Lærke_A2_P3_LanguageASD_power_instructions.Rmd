...
```{r loading data}
pacman::p_load(simr, lme4, tidyverse)
...
```{r}
df$Child.ID <- as.character(as.factor(df$Child.ID))
...
# The arrow error seems to occur when Child.ID is of the class character. But should it then be numeric or factor???

# We probably don't need DiagnosisTD:Visit in the interaction in the "fixed()" functions, because then we get errors, and the powercurve is flat.
```
...
pDV1 <- powerSim(m2, fixed("Diagnosis:Visit"), nsim = 50)
pcDV1 <- powerCurve(m2, fixed("Diagnosis:Visit"), along = "Child.ID", nsim = 50)
pDV1
plot(pcDV1)
...
```{r}
m2 <- lme4::lmer(CHI_MLU ~ Diagnosis * Visit + (1 + Visit + I(Visit^2) | Child.ID), df, REML = FALSE, lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE)) # This is the same model as m, but I just put it here, because we are going to tamper with it.
...
fixef(m)["DiagnosisTD:Visit"] <- 0.15
...
pDV_2.2 <- powerSim(m2, fixed("DiagnosisTD:Visit"), nsim = 50)
pcDV_2.2 <- powerCurve(m2, fixed("DiagnosisTD:Visit"), along = "Child.ID", nsim = 50)
pDV_2.2
plot(pcDV2)
